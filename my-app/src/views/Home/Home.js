...
const Home = () => {
    ...
    if (id === 0) {
      ...
      try {
        const res = await axios.post(
          "http://localhost:5000/MInsertBanner",
          formData
        );
  ...
  const edithandler = async (id) => {
    ...
    axios.get(`http://localhost:5000/Mfinddata/${id}`, {
      method: 'GET',
      // headers: {
  ...
  function getdata() {
    axios.get(`http://localhost:5000/Mfinddata`)
      .then(function (res) {
  ...
  const deletehandler = async (id) => {
    swal(
      ...
      .then((willDelete) => {
        if (willDelete) {
          axios.delete(`http://localhost:5000/MDeleteBanner/${id}`)
            .then(res => {
              getdata();
            }
            )
    ...
    <>
      ...
      <CModal visible={visible} onClose={() => setVisible(false)} >
        ...
        <CModalBody>
                ...
                <CForm action="/MInsertBanner" method="POST">
                  <div className="mb-3">
                    ...
                    <CFormInput type="text" id="exampleFormControlInput1" value={First_Name} onChange={(e) => { setFirst_Name(e.target.value) }} placeholder="Enter Firstname" />
                  </div>
                  <div className="mb-3">
                    <CFormLabel htmlFor="exampleFormControlTextarea1">Last Name</CFormLabel>
                    <CFormInput  type="text" value={Last_Name} onChange={(e) => { setLast_Name(e.target.value) }} placeholder="Enter Lastname" rows="3"></CFormInput>
                  </div>
                  <div className="mb-3">
                    <CFormLabel htmlFor="exampleFormControlTextarea1">User Name</CFormLabel>
                    <CFormInput type="text" value={User_Name} onChange={(e) => { setUser_Name(e.target.value) }} placeholder="Enter Username" rows="3"></CFormInput>
                  </div>
                  <div className="mb-3">
        ...
        <CModalFooter>
          ...
          <CButton className="btn1" onClick={submit} id="submit">Save</CButton>
        </CModalFooter>
      </CModal>
        ...
        <CCard className="mb-4">
          ...
          <CCardBody>
            <CInputGroup className="flex-nowrap">
              ...
              <CFormInput placeholder="Search"  value={search} onChange={(e) => { setSearch(e.target.value) }} aria-label="Username" aria-describedby="addon-wrapping" /><CButton style={{ marginLeft:"650px",borderRadius:"5px"}} className="btn1" onClick={() => setVisible(!visible)}>Add Movie Make</CButton>
            </CInputGroup>
            <br></br>
            ...
            <CTable style={{ textAlign: "center" }}  hover>
              ...
              <CTableBody>
                {
                  list.filter(data => data.First_Name.match(new RegExp(search, "i"))).reverse().map((item, i) => {
                    ...
                    return (<>
                      <CTableRow key={i} style={{backgroundImage: 'linear-gradient(to right,#16222A,#3A6073)'}}>
                        ...
                        <CTableDataCell  style={{ paddingTop: "20px",color:"white" }}>{item.First_Name}</CTableDataCell>
                        <CTableDataCell  style={{ paddingTop: "20px",color:"white" }}>{item.Last_Name}</CTableDataCell>
                        <CTableDataCell  style={{ paddingTop: "20px",color:"white" }}>{item.User_Name}</CTableDataCell>
                        <CTableDataCell  style={{ paddingTop: "20px",color:"white" }}>{item.Email}</CTableDataCell>
